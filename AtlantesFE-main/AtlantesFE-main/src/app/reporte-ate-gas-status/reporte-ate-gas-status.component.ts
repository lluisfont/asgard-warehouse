...
@Component({
    selector: 'app-reporte-ate-gas-status',
    templateUrl: './reporte-ate-gas-status.component.html',
    styleUrl: './reporte-ate-gas-status.component.css',
    providers:[UsuarioService,DatoMaestroService,AlmacenesService,ExportExcelService]
})
export class ReporteAteGasStatusComponent {
    public token: string;
    ...
    constructor(
        ...
        if(this.tokenDetalle.idtipousuario==1){
            this.ver_reporte_ate_gas_status=true;
        }else{
            let indiceVerReporte= this.tokenDetalle.permisos.findIndex(x => x.idmodulo === 108);
    ...
    generarReporte(){
        ...
        if(!error){
            this.generado=true;
            this._almacenesService.reporteategasstatus(this.token, this.idcliente, this.fechainicial, this.fechafinal).subscribe(
                response => {
                    this.generado=false;
                    this.reporte=this.normalizarReporte(this.resolverDataReporte(response, 'reporte_status'));
                    this.columnas=this.generarColumnas(this.reporte);
                    this.reportexlsx=this.generarExcel("Reporte de Status");
                },
                error => {
                    this.generado=false;
                    console.log(<any>error)
