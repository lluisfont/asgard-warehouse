...
@Component({
    selector: 'app-reporte-ate-gas-salidas',
    templateUrl: './reporte-ate-gas-salidas.component.html',
    styleUrl: './reporte-ate-gas-salidas.component.css',
    providers:[UsuarioService,DatoMaestroService,AlmacenesService,ExportExcelService]
})
export class ReporteAteGasSalidasComponent {
    public token: string;
    ...
    constructor(
        ...
        if(this.tokenDetalle.idtipousuario==1){
            this.ver_reporte_ate_gas_salidas=true;
        }else{
            let indiceVerReporte= this.tokenDetalle.permisos.findIndex(x => x.idmodulo === 107);
    ...
    generarReporte(){
        ...
        if(!error){
            this.generado=true;
            this._almacenesService.reporteategassalidas(this.token, this.idcliente, this.fechainicial, this.fechafinal).subscribe(
                response => {
                    this.generado=false;
                    this.reporte=this.normalizarReporte(this.resolverDataReporte(response, 'reporte_salidas'));
                    this.columnas=this.generarColumnas(this.reporte);
                    this.reportexlsx=this.generarExcel("Reporte de Salidas");
                },
                error => {
                    this.generado=false;
                    console.log(<any>error)
