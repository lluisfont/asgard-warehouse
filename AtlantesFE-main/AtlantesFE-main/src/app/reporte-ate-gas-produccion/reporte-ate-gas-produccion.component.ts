...
@Component({
    selector: 'app-reporte-ate-gas-produccion',
    templateUrl: './reporte-ate-gas-produccion.component.html',
    styleUrl: './reporte-ate-gas-produccion.component.css',
    providers:[UsuarioService,DatoMaestroService,AlmacenesService,ExportExcelService]
})
export class ReporteAteGasProduccionComponent {
    public token: string;
    ...
    constructor(
        ...
        if(this.tokenDetalle.idtipousuario==1){
            this.ver_reporte_ate_gas_produccion=true;
        }else{
            let indiceVerReporte= this.tokenDetalle.permisos.findIndex(x => x.idmodulo === 109);
    ...
    generarReporte(){
        ...
        if(!error){
            this.generado=true;
            this._almacenesService.reporteategasproduccion(this.token, this.idcliente, this.fechainicial, this.fechafinal).subscribe(
                response => {
                    this.generado=false;
                    this.reporte=this.normalizarReporte(this.resolverDataReporte(response, 'reporte_produccion'));
                    this.columnas=this.generarColumnas(this.reporte);
                    this.reportexlsx=this.generarExcel("Reporte de Produccion");
                },
                error => {
                    this.generado=false;
                    console.log(<any>error)
